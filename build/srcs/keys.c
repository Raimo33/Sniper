...
void init_keys(keys_t *restrict keys)
{
  ...
  fast_assert(priv_key && api_key, STR_AND_LEN("Missing keys"));

  memcpy(keys->sender_comp_id, STR_AND_LEN(CLIENT_COMP_ID));
  ...
  fast_assert(EVP_PKEY_id(keys->priv_key) == EVP_PKEY_ED25519, STR_AND_LEN("Invalid private key type"));
}
...
void generate_ws_key(uint8_t *restrict key)
{
  ...
  fast_assert(encoded_size == WS_KEY_SIZE, STR_AND_LEN("Failed to encode ws key"));
}
...
bool verify_ws_key(const uint8_t *restrict key, const uint8_t *restrict accept, const uint16_t len)
{
  ...
  fast_assert(decoded_size == WS_KEY_SIZE, STR_AND_LEN("Failed to decode ws key"));
